...
8×8 격자에서 인접한 두 보석을 교환해 같은 색 3개 이상을 가로 또는 세로로 만들면 제거.
제거된 자리는 위에서 새 보석이 떨어져 채워지며, 연속 매치가 일어나면 콤보 배율이 적용됨.
60초 제한 시간 안에 최대한 많은 점수 획득.

### 조작

| 입력 | 동작 |
|---|---|
| 보석 클릭 | 첫 보석 선택 (외곽선 표시) |
| 인접 보석 클릭 | 교환 시도 — 매치 발생 시 진행, 아니면 셰이크 후 되돌림 |
| 비인접 보석 클릭 | 선택을 새 보석으로 변경 |
| 같은 보석 재클릭 | 선택 해제 |

### 점수 시스템

매치 발생 시: `제거된 셀 수 × 10 × 콤보 배율`

콤보 배율은 한 번의 스왑이 일으킨 연쇄 깊이:
- 1차 매치 ×1, 2차 매치 ×2, 3차 매치 ×3, ...

가능한 스왑이 모두 막히면 자동 셔플 ("셔플!" 표시).

### 주요 구현

- **그리드**: `grid[r][c]` 0~5 색상 ID, 제거 직후 잠깐 `-1`(빈 칸)
- **매치 감지** (`findMatches`): 가로/세로 각각 런-길이 스캔 → `marks[r][c]` 불리언 격자로 합집합 처리 (L/T자 매치 자동 처리)
- **연쇄 루프** (`runCascade`): 매치 → 점수 → 팝 애니메이션 → `-1` 처리 → 중력+리필 → 다시 매치 검사. 매치 없을 때까지 반복하며 콤보 증가
- **중력+리필** (`applyGravityAndRefill`): 열별로 바닥부터 `writeR` 포인터를 올리며 비어있지 않은 셀을 아래로 모음, 남은 위 칸은 새 랜덤 색
- **데드락 검사** (`hasAnyMove`): 인접한 모든 쌍에 대해 가상 스왑 → 매치 검사 → 복귀. 가능한 수가 없으면 `reshuffle()`
- **초기 보드** (`safeColor`): 좌·위 2칸을 참조하며 매치 만들지 않는 색만 선택
- **입력 잠금**: `locked` 플래그 + `async/await sleep`로 애니메이션 시퀀스 보호
- **DOM 전략**: 64개 셀 div를 한 번 생성, 이후 `gem.dataset.color`만 갱신

### localStorage

| 키 | 값 |
|---|---|
| `hs_match3_best` | 최고 점수 (정수) |
